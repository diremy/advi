...
let input_preamble ic =
  ...
  then raise (Failure "invalid jfm file");
...
let input_header ic len =
  let read = ref 0 in
  if len < 2 then raise (Failure "too short header");
  ...
  let sbsf, w, s, t =
    ...
    end else None, None, None, None
  in
  if len - !read > 0 then begin
    ignore (input_string ic (len - !read))
  end;
  { checksum = cs;
    design_size = ds;
...
*)
  ...
  with
    End_of_file -> ()
  | Exit -> raise (Failure "jfm loading failed")
  end;
  close_in ic;
(*
  prerr_endline "done";
