...
Here is a simplistic talk example, to begin with. As a first simple
interaction with the presenter, write {\tt \char92pause} into your
{\LaTeX} code when you want the presenter to stop when it is
displaying a slide.
...
\firstslide{My talk}\centerline{{\large \bf \em Me Myself}}
\newslide{Plan}
 Nparts in this talk: Part 1, Part 2, and Partn.
\newslide{Part 1}
...
\newslide{Part n}
 Blz blz.
\lastslide{Conclusion}
 This was my first talk.
\end{document}
\end{verbatim}

More involved examples (including Makefiles to compile the whoe thing)
can be found in the directory {\tt examples} of the distribution. (In
particular {\tt examples/slitex}.)
...
the default \doctt{~/.advirc} or \doctt{~/.advi/advirc} init files).
...
\verb"-v"         & Prints the {\tt advi} current version and exits \\
\verb"--version"  & Prints the full {\tt advi} current version and
exits \\
...
launches a new \verb"advi" process to view the target.
...
The package \softlink{advi}{{\tt advi}} below redefines the macro
...
\begin{quote}
This is the default case: the application is specific to a
given page. Launched whenever the page is displayed, the application is
killed when the page is turned.
\end{quote}

\dockey{persistent}{name}

\begin{quote}
Launched only once, the application keeps running in the background,
but is only visible on the page where it has been launched.
\end{quote}

\dockey{sticky}{name}
\begin{quote}
Launched only once, the application keeps running and remains visible
when turning pages. It is also resized and moved as necessary to fit
the page size.
\end{quote}

\dockey{raw}{name}
\begin{quote}
The application is launched each time its embedding command is
encountered. A \doctt{raw} application is not managed by {\ActiveDVI},
except for the initial launching and the final clean-up that occurs
when {\ActiveDVI} exits. You can monitor \doctt{raw} applications with
\doctt{advikillembed} and the associated window mapping facilities
for \doctt{raw} applications (see below).
\end{quote}
...
\begin{quote}
Map the window of the embedded applications named \docid{name}.
\end{quote}

\medskip\noindent
\docdef \adviunmapembed \docarg{name}
\begin{quote}
Unmap the window of the embedded applications named \docid{name}.
\end{quote}
...
\begin{quote}
Map the windows of all the embedded applications named \docid{name}.
\end{quote}

\medskip\noindent
\docdef \adviunmapallembed \docarg{name}
\begin{quote}
Unmap the windows of all the embedded applications named \docid{name}.
\end{quote}
