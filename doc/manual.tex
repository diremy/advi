...
\paragraph{Warning!} Most commands of \verb$advi.sty$ use 
the {\TeX} \verb"\special" command to insert special commands into the DVI
output file, which commands are then interpreted by {\ActiveDVI} when while
previewing.  Note that \verb$\special{bla bla}$ is equivalent to
\verb$\hbox{}$ for {\TeX}'s mounth, which may alters the document layout
accordingly.  Hence, be aware that most commands of \verb$advi.sty$ will
also be equivalent to \verb$\hbox{}$ regarding the document layout.
...
encountered. A \doctt{raw} application is not {\em automatically}
managed by {\ActiveDVI}, except for the initial launching and the
final clean-up that occurs when {\ActiveDVI} exits. You can monitor
\doctt{raw} applications with \doctt{advikillembed} and the associated
window mapping facilities for \doctt{raw} applications (see below).
\end{quote}
