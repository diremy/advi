...
exchanging the background and foreground colors \\
...
you may move over \softlink{activedvi}{this area} 
or click on \softlink{otherpage}{this one} to see
the effect.)
...
reserved hyper-targets markers around the table of contents. It also
...
\label{multiplex}
{\ActiveDVI} can be invoked with several DVI files (currentlt only two).
In this case, the first file is the master file and others are client
files. The user can switch between files explicitly (see 
\softlink{activedvikeys}{key bindings}) or implicitly.
There is an implicit switch from master to client when an hyperlink that is
not found in master file can be found in the client; there is 
an implicit switch from client to master when using the history stack and
the previous event on the stack an implicit swtich from master.
...
\end {itemize}
However, some other PStricks may not to work.
...
\softtarget{activedvikeys}{\section{Key bindings}}
...
\softtarget{writingkeys}{\section{Key bindings for scratch writings}}

\input{scratch_write_splash_contents.tex}

\softtarget{drawingkeys}{\section{Key bindings for scratch drawings}}
