...
engine. \\
Alexandre~Miquel wrote Mldvi and distributes it under the LGPL
license at URL\\ {\url{http://pauillac.inria.fr/~miquel/}}.

{\ActiveDVI} has been developed by Jun~Furuse, Didier~R{\'{e}}my, and
Pierre~Weis, with contributions by Roberto~Di~Cosmo, Didier~Le~Botlan,
Xavier~Leroy, and Alan~Schmitt.

\clearpage
%\advitoc
\hypertarget{/toc.first}{}
\tableofcontents
\hypertarget{/toc.last}{}
%\endadvitoc
\clearpage

\section{Installation}

After successful installation of {\ActiveDVI}, don't forget to have a
...
port of the {\ActiveDVI} binaries; in such a case, use the recommended
...
installation of this library, download and extract the {\ActiveDVI}
...
{\ActiveDVI} allows (almost) any {\LaTeX} piece of code to be
recorded, and have the corresponding DVI code to be rendered later
upon request. This ability to "render later" requires the {\LaTeX}
code recorded to be tagged: this defines the notion of an {\em
{\ActiveDVI}-record}.
...
then binds it to the tag \docid{tag}. While recording, the DVI output is
not displayed unless the option \doctt{play} is set.

{\ActiveDVI}-records can be nested. If so, the inner record is bound
to its own tag as usual; if the inner record is defined with the
\doctt{play} option, it is also recorded as a part of the outer tag
record.
...
\begin{quote}
Replays the previously recorded and bound to \docid{tag} DVI code.
...
An {\em {\ActiveDVI}-anchor} is a specific area in the page that can
be {\em activated} when some specified mouse events occur.  The anchor
is associated to an {\ActiveDVI}-record that is automatically rendered
each time the anchor is activated. The anchor is {\em de-activated}
when the event that triggered the activation does not hold any
more. The page is reset to its original appearance when the anchor is
de-activated.
...
\begin{quote}
Defines the DVI rendering of \docarg{text} as an {\ActiveDVI}-anchor
that plays the {\ActiveDVI}-record bound to \docid{tag} when the anchor is
activated.

The argument \docid{activation} specifies the activation method of the
anchor: it may either be \doctt{over} or \doctt{click}. If the
activation method is \doctt{over}, the anchor is activated whenever the
mouse pointer is inside the anchor area; the anchor is de-activated when
the mouse leaves the anchor area. If the activation method is
\doctt{click}, the anchor is activated whenever the button is pressed inside
the anchor area; the anchor is de-activated when the button is
released.
...
Images can be lighten by specifying an alpha value (a
floating point number between 0 and 1) that measures the mixing
between the background and the image.

Images can also be blended, meaning that you can choose the
algorithm that sur-imposes the image to the background.
Blending modes are reminiscent of Ghostscript blending options: blend
mode should be one of the following: {\tt normal}, {\tt multiply},
...
\subsection{\label {advi-grahicx}The {\doctt{advi-graphicx}} {\LaTeX} package}
