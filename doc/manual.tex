...
\begin{abstract}
\ActiveDVI~is a presentation graphics program to provide the highest
quality slides presentation experience to \LaTeX~users.

\ActiveDVI~is a viewer and presenter for DVI files that is targeted to
presentations via laptop computers: \ActiveDVI~uses the computer
processing power to offer various special effects to animate the
presentation.

\ActiveDVI~can be used as is, but will shine when driven by a user
with a bias towards programming: new special effects can easily be
added and creative advanced users like to program the presenter at
various levels, either via \LaTeX~macros or \LaTeX~package
files. Brave souls can even directly modify \ActiveDVI's source code
since that code is freely available.

Presentation examples can be found in the \verb"test" and
\verb"examples" directories. Don't miss to play them! Then, feel free
...
communauty, please don't hesitate to drop a mail to the implementors
...
\section {The {\tt advi.sty} \LaTeX~packages}

\ActiveDVI~provides some \LaTeX~packages to facilitate animations and
interaction with the presenter from within \LaTeX.

The {\tt advi} package is the main package to use when writing
presentations for \ActiveDVI. It provides the main set of interactive
commands for {\ActiveDVI}. However, there is no need to load the
package if no {\ActiveDVI} special effects are required for the
presentation.

\subsection{Printing the presentation}

The {\tt advi} package recognizes the special option \verb"ignore", which
is devoted help producing a printable version of the presentation: the
\verb"ignore" option makes the package not to produce {\tt advi}
specials, so that the show can be previewed by other previewers or
turned into Postscript with {\tt dvips}. Of course, this option
disables most effects that cannot be printed, although some of them
are still approximated.
...
synchronization between {\tt ghostscrip} and \ActiveDVI~drawings 
uncontrolable. In this case, the interleaving of inlined postscript 
...
If you use those versions of ghostscript, inline specials should be
correctly rendered.
...
\url{http://cristal.pauillac.inria.fr/~remy/ghostscript/}.
