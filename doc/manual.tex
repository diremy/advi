...
quality slides presentation experience to the $\LaTeX$ users.
...
a general interest, or if you can provide a nice or fancy presentation
...
This package provides interactive commands for {\ActiveDVI}.  There is
no need to load the package if those {\ActiveDVI} special effects are
not required. The package recognizes the option \verb"ignore", which
makes it not to produce any {\tt advi} specials, so that the show can
be previewed by other previewers or turned into Postscript with {\tt
dvips}. Of course, this option disables most effects, although some of
them are still approximated.
