...
it locally with the commands {\docdef \adviignore}.
The package also defines the conditional
{\docdef \ifadvi} which
evaluate its first  argument if advi is not ignore and its second argument
otherwise. 
...
\item
Since caracters are rendered by{\ActiveDVI}, some PStricks are not allowed.
\item
