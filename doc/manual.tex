...
\usepackage{hevea}
%HEVEA\def\strech #1{2em}
%HEVEA\def\vfill {2em}
%HEVEA\def\hfill {2em}
%HEVEA\def\vfil {2em}
%HEVEA\def\space { }
%HEVEA\def\sqrt#1{\mathbin{square}(#1)}
...
\section{Using the \softtarget{otherpage}{\ActiveDVI} previewer}
...
\end{verbatim}
See section~\ref{multiplex} for the use of a secondary DVI file.
...
implicit switch to master \\& on \texttt{usr1} signal (default if off).\\
\end{tabular}
...
enter the {\em scatch setting mode} to set various properties of the
...
There is an implicit switch from the master to client file $c$ when an
hyperlink that is not found in master file can be found in the client file
$c$; there is also a switch from client $c$ to master when using the
history stack and the previous event on the stack was an implicit switch
from master to client $c$. 

If \doctt{autoswitch} flag is set, there is also an implicit switch to
master, whenever {\ActiveDVI} receives signal \doctt{usr1} (to mean 
immediate refresh). 


\section{The \softtarget{advistyle} {\doctt{advi.sty}} {\LaTeX} package}
...
which colors are available look at the source file of the package
...
{\ActiveDVI} can deal with most of PStricks by calling {\tt ghostscript} on
included Postscripts. Basic change of coordinates are implemented, but this
feature remain fragile, as {\ActiveDVI} must in turn call {\tt ghostscript}
to get the new coordinates. Also, rotations will definitely not work for
text, which is rendered by {\ActiveDVI} and cannot be rotated.
...
\section{The \softtarget{advi-slides} {\doctt{advi-slides.sty}} {\LaTeX} package}

{\ActiveDVI} provides a specialized {\LaTeX} package to facilitate
writing presentation slides in the spirit of SliTeX.
...
\section{\softtarget{activedvikeys}{Key bindings}}
...
\section{\softtarget{writingkeys}{Key bindings for scratch writings}}

\input{scratch_write_splash_contents.tex}

\section{\softtarget{drawingkeys}{Key bindings for scratch drawings}}
