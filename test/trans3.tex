...
\verb"\advitransbox{path,steps=10000,startx=500,starty=1000,stopx=10,stopy=10,genpath=line}"

but we should make sure that, if start or stop is omitted, it is replaced by the actual coordinates
of the box in the dvi screen, or we could add a special keyword to be parsed as boxx, boxy to be used
in start and stop directives.

{
\Huge
\begin{itemize}
\item \textcolor{red}{From \advitransbox{path,steps=10000,startx=500,starty=1000,stopx=10,stopy=10,genpath=line}{path}}
