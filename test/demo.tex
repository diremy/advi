...
width and height specification \verb"w" and \verb"h"; \verb"command"
will be display in the window \verb"!p" (normally a sub-window of
{\ActiveDVI}'s window).
...
presentation:
\begin{itemize}
\item {\em embeddable applications} run inside a
subwindow of the main {\ActiveDVI}'s window (hence their window is
stuck to the precise place computed by {\ActiveDVI}),
\item {\em non-embeddable applications} run in a window of
their own (hence their window is unrelated to the {\ActiveDVI}'s window and
...
specify the root window may vary depending of the application at hand
...
Rerootable applications obeys to the X geometry specifications and can
be properly displayed within the {\ActiveDVI} window (embedding).
...
%\adviembed[width=15.93cm,height=12.88cm]{mplayer -geometry !wx!h dancemonkeyboy.mpg}
\end{center}
...
For the time being, the different viewer commands can be set using an
option on the command line (or including the same option in a
\verb".advirc" file).
...
\hyperref{../doc/manual.ps}{}{}{\tt viewer for PostScript files} (by
default {\tt gv}),
...
\item A warning is emitted on standard error (Unix {\tt stderr}), if
the file extension is not recognized:

\hyperref{bad-talk.ppt}{}{}{\tt bad-talk.ptt}

or the file is non-existent:
...
\noindent
Another form of simple annotations is the overriding of words.
...
\begin{superpose}
\advirecord[play]{1}{pieces \advianchor{2}{of} \advianchor{3}{text}}\\
