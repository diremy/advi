...
\verb|circgradient| predefined functions, we can convert the entire background
color into a color gradient from the chosen color to white.

Additionally the gradient colors and the area where the gradient is drawn can be
specified:
\begin{itemize}
\item \verb"colorstart" specify the starting color of the gradient,
\item \verb"colorstop" specify the ending color of the gradient,
...
Dimensions of the area are floating point numbers in the range
\verb|[0 .. 1]| that gives the ratio of the area with respect to the
page ($0$ meaning a null dimension and $1$ the entire actual
page relevant dimension).

For centered or circular gradient, \verb"xcenter" and \verb"ycenter"
define the coordinates of the center point of the gradient (those are
also dimensions with the same encoding as in the preceding paragraph).
...
\begin{quote}
  \begin{verbatim}
  \advibg{color = cyan, fun = "cgradient",
          colorstart = "blue"} 
  \end{verbatim}
\end{quote}
\vfill
...
\begin{quote}
  \begin{verbatim}
\advibg{color = [named]{MistyRose}, fun = "circgradient",
        colorstart = red,
        colorstop = [named]{MistyRose},
        width = 0.2,
        height = 0.15,
