...
let color_major = ref "Red"

(** Color of the minor hand. *)
let color_minor = ref "Blue"

(** Width of the major hand. *)
let width_major = ref 3

(** Width of the major hand. *)
let width_minor = ref 2

(** Width of the circle. *)
let width_circle = ref 2
...
let total_time = ref 1800.0

(** Color for the authorized time interval. *)
let color_ok = ref "Green"

(** Color for the done time interval. *)
let color_done = ref "Blue"
...
module Args = struct
  ...
  let options = [
    "-d", Arg.Int (fun d -> diametre := d), "<d>  Use <d> as the diameter of the clock" ;
    "-wc", Arg.Int (fun n -> width_circle := n), "<n>  Use <n> as the width of the circle line";
    "-W", Arg.Int (fun n -> width_major := n), "<n>  Use <n> as the width of the major hand";
    "-w", Arg.Int (fun n -> width_minor := n), "<n>  Use <n> as the width of the minor hand";
    "-C", Arg.String (fun s -> color_major := s), "<color>  Set the color name used for the major hand";
    "-c", Arg.String (fun s -> color_minor := s), "<color>  Set the color name used for the minor hand";
    "-B", Arg.Float (fun f -> base_major := f), "<base>  Set the value for 360° of the major hand  (float, in milliseconds)" ;
    "-b", Arg.Float (fun f -> base_minor := f), "<base>  Set the value for 360° of the minor hand  (float, in milliseconds)" ;
    "-rev", Arg.Set reverse, " Countdown style";
    "-time", Arg.Int (fun m -> total_time := float_of_int (m * 60000)), "<time>  Theorical total time in minutes" ;
    "-f", Arg.Float (fun f -> frequency := f), "<f>  Set the refresh frequency (in milliseconds, default is 1000.0)";
    "-file", Arg.String (fun f -> filename := Some f), "<file>  Use <file> to read the current slide number when receiving SIGUSR1" ;
  ...
  let parse () = Arg.parse 
	...
	with Invalid_argument err -> 
      ...
      ("usage : "^Sys.argv.(0)^" [options] t1 t2 ... (in seconds)\nOptions are:")
end
...
let options_circle () = [Width (Pixels !width_circle)]
let options_major () = [Width (Pixels !width_major) ; FillColor (NamedColor !color_major)]
let options_minor () = [Width (Pixels !width_minor) ; FillColor (NamedColor !color_minor)]
...
let draw_major c = 
  ...
  c.major <- Canvas.create_line c.widget [pr; pr; Pixels (r+x); Pixels(r+y)] (options_major ())
...
let draw_minor c = 
  ...
  c.minor <- Canvas.create_line c.widget [pr; pr; Pixels (r+x); Pixels(r+y)]  (options_minor ())
...
let create_clock () =
  ...
  let clock = 
    {
      widget = w ;
      circle = Canvas.create_oval w (Pixels 0) (Pixels 0) (Pixels d) (Pixels d) (options_circle());
...
let draw_overtime_arc c old_major =
  ...
  let arc = Canvas.create_arc c.widget (Pixels 0) (Pixels 0) (Pixels d) (Pixels d)
      [ Start angle_inf ; Extent angle_sup ; FillColor (NamedColor !color_overtime) ;
...
let create_secteurs clock times =
  ...
  | t :: q ->
	...
	(
	 prerr_endline "The sum of the given times is greater than the total time." ;
	 []
	)
      else
	(
	 ...
	 Printf.printf "overtime inf=%f sup%f new_acc=%f" angle_inf angle_sup new_acc;  print_newline ();
	 let arc = Canvas.create_arc clock.widget (Pixels 0) (Pixels 0) (Pixels d) (Pixels d)
	     [ Start angle_inf ; Extent angle_sup ; FillColor (NamedColor !color_ok) ;
...
let rec time freq c () =
  ...
  c.major_elapsed <- 
    (if !reverse then
      c.major_elapsed -. gain
    else
      c.major_elapsed +. gain
    );

  if c.major_elapsed > !total_time or 
    c.major_elapsed < 0.0 
  then
    draw_overtime_arc c old_major
  else
    ();
  ...
  draw_major c;
  last_time := new_time ;
  ignore (Timer.add (int_of_float freq) (time freq c))
;;
...
let signal clock =
  let must_colorize =
	...
	try
	  ...
	  let n = int_of_string (input_line chanin) in
  ...
  if must_colorize then
    try
      ...
      Canvas.configure_arc clock.widget sec [ FillColor (NamedColor !color_done)]
    with
      _ ->
	()
  else
...
Sys.set_signal Sys.sigusr1 (Sys.Signal_handle (fun _ -> signal clock));;

ignore (Timer.add (int_of_float !frequency) (time !frequency clock));;
